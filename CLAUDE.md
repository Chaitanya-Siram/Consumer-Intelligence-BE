...
- **Caching**: payload stored on S3 at `session_files/session_{id}/ci_charts_data/latest.json` (+ timestamped copy) because the session table has no CI column and models must not change. `refresh=true` bypasses
- Article gate matches MI: `is_approved_for_dashboards` rows if any approved, else all `is_relevant` rows
- WS message types: `start`, `progress`, `lens_complete` (carries finished storyboard), `lens_error`, `complete`, `error`
