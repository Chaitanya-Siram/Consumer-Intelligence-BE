...
- Tier-2 lenses shipped: `track_emerging_issues` (`storyboard/emerging_issues.py`, issue = theme group ranked by growth + negativity + unmet needs + brand share) and `shifting_audience_priorities` (`storyboard/audience_priorities.py`, loyalty index 10–100 from weighted bucket shares; `WEIGHTS` is a placeholder until deck values arrive). Contract: `Consumer-Intelligence-FE/docs/ci-lens-contracts-issues-priorities.md`
- **Caching is incremental**: payload stored on S3 at `session_files/session_{id}/ci_charts_data/latest.json` (+ timestamped copy). `router._plan` compares cached lens keys with the session's required keys and rebuilds only the missing ones, merging via `_merge_payload` (drops stale coming-soon stubs). `refresh=true` rebuilds the requested lenses but keeps other cached ones, so `?lenses=x&refresh=true` refreshes one lens safely
- Adding a Tier-2 lens: builder module with `LENS_KEY` + `build_storyboard()` → register in `builder._MODULES/_ORDER/_HERO_QUERY` → facts/schema/apply + `_REGISTRY` entry in `narrative.py` → `tier_registry.TIER1_TO_LENS_KEYS` and `CI_LENS_KEYS` → FE `CI_LENS_KEYS`/`TIER1_TO_CI_KEYS`, screen, route, `tierLensData` `route:`
